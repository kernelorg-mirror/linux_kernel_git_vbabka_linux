...
enum stat_item {
	...
	SHEAF_FLUSH_OTHER,	/* Objects flushed from other sheafs */
...
static __fastpath_inline
unsigned alloc_from_pcs_bulk(struct kmem_cache *s, size_t size, void **p)
{
	struct slub_percpu_sheaves *pcs;
...
next_batch:
	...
	if (unlikely(pcs->main->size == 0)) {
		...
		/*
		 * Once full sheafs in barn are depleted, let the bulk
 ...
 * Unlike free_to_pcs() this has includes the calls to all necessary hooks
...
no_empty:
		...
		/*
		 * if we depleted all empty sheafs in the barn or there are to
		 * many full sheafs, free the rest to slab pages
		 */
fallback:
				 ...
				 void **p)
{
	...
	if (i < size) {
		unsigned j = __kmem_cache_alloc_bulk(s, flags, size - i, p + i);
		/*
...
int __kmem_cache_shutdown(struct kmem_cache *s)
{
	...
	// TODO: some rcu_free sheaves might be in flight, can we
	// synchronize_rcu() here or not because of cpu hotplug and slab_mutex
	// locked?
...
static int slab_mem_going_online_callback(void *arg)
{
	...
	list_for_each_entry(s, &slab_caches, list) {
		...
		if (s->cpu_sheaves) {
			...
			barn = kmalloc(sizeof(*barn), GFP_KERNEL);
			if (!barn) {
				kmem_cache_free(kmem_cache_node, n);
				ret = -ENOMEM;
				goto out;
...
/**
 * kmem_cache_setup_percpu_sheaves - Create percpu sheaves for the cache.
 * @s: The cache to add sheaves to. Must be created with SLAB_NO_MERGE flag.
 * @capacity: Object capacity of each sheaf.
 * @rcu_dtor: Optional function to be called after kfree_rcu() grace period.
 *
 * After this call, allocations from the cache go through caching arrays called
 * sheaves. Each cpu has a main sheaf that's always present, and a spare sheaf
 * thay may be not present. When both become empty, there's an attempt to
 ...
 * allocation falls back to the normal operation allocating a single object from
 * a slab.
 *
 * Analogically when freeing and both percpu sheafs are full, the barn may
 * replace it with an empty sheaf, unless it's over capacity. In that case
 * a sheaf is bulk freed to slab pages.
 *
 * The sheaves does not distinguish NUMA placement of objects, so allocations
 * via kmem_cache_alloc_node() with a node specified other than NUMA_NO_NODE
 * will bypass them.
 *
 * Bulk allocation and free operations also try to use the cpu sheaves and barn,
 * but fallback to using slab pages directly.
 *
 ...
 * between sheafs and slab pages, which may result in e.g. not detecting a
 * use-after-free while the object is in the array cache, and the stacktraces
 * may be less useful.
 *
 * Return: 0 if OK, -EINVAL on caches without SLAB_NO_MERGE or with the sheaves
 * already created, -ENOMEM when the per-cpu sheaves creation fails.
 */
int kmem_cache_setup_percpu_sheaves(struct kmem_cache *s, unsigned int capacity,
				    void (*rcu_dtor)(void *))
{
	int nid, cpu;
	struct kmem_cache_node *n;
	...
	for_each_node_mask(nid, slab_nodes) {
		struct node_barn *barn;
