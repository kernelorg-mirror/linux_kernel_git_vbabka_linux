...
{
	const struct page *page = slab_page(slab);
					...
					void *object, u8 val)
{
	...
	if (s->flags & SLAB_RED_ZONE) {
		...
		if ((s->flags & SLAB_POISON) && s->object_size < s->inuse) {
			check_bytes_and_report(s, slab, p,
					       "Alignment padding",
...
static
void setup_page_debug(struct kmem_cache *s, struct slab *slab, void *addr)
{
...
{
	struct slab *slab;slab = container_of(h, struct slab, rcu_head);
...
{
	struct slab *slab;struct slab *slab2;
	void *object = NULL;
	...
	do {
		...
		for_each_zone_zonelist(zone, z, zonelist, highest_zoneidx) {
			...
			if (n && cpuset_zone_allowed(zone, flags) &&
				...
				object = get_partial_node(s, n, ret_slab,
							  flags);
				if (object) {
					/*
...
#ifdef CONFIG_SLUB_CPU_PARTIAL
static void __unfreeze_partials(struct kmem_cache *s,
				struct slab *partial_slab)
{
	struct kmem_cache_node *n = NULL, *n2 = NULL;
	struct slab *slab;struct slab *slab_to_discard;
	unsigned long flags = 0;

	slab_to_discard = NULL;

	while (partial_slab) {
		struct slab new;struct slab old;slab = partial_slab;
...
static void unfreeze_partials_cpu(struct kmem_cache *s,
				  struct kmem_cache_cpu *c)
{
	struct slab *partial_slab;partial_slab = slub_percpu_partial(c);
	c->partial = NULL;
...
static void put_cpu_partial(struct kmem_cache *s, struct slab *slab,
			    int drain)
{
	struct slab *oldslab;struct slab *slab_to_unfreeze;
	unsigned long flags;
	int slabs;
	int pobjects = 0;

	slab_to_unfreeze = NULL;
	slabs = 0;
 ...
 */
static inline bool pfmemalloc_match_unsafe(struct slab *slab, gfp_t gfpflags)
{
	if (unlikely(slab_test_pfmemalloc_unsafe(slab)))
		return gfp_pfmemalloc_allowed(gfpflags);
...
redo:
	 ...
	 */
	if (unlikely(!pfmemalloc_match_unsafe(slab, gfpflags)))
		goto deactivate_slab;
...
retry_load_page:
	...
	if (unlikely(c->slab)) {
		...
		struct slab *flush_slab;flush_slab = c->slab;
...
void kmem_cache_free_bulk(struct kmem_cache *s, size_t size, void **p)
{
	...
	do {
		...
		slab_free(df.s, df.slab, df.freelist, df.tail, df.cnt,
			  _RET_IP_);
	} while (likely(size));
}
...
{
	LIST_HEAD(discard);
	struct slab *slab;struct slab *h;
...
void kfree(const void *x)
{
	...
	/* XXX: could benefit from __PageSlab() like virt_to_slab() */
	if (unlikely(!PageSlab(page))) {
...
static int __kmem_cache_do_shrink(struct kmem_cache *s)
{
	...
	int i;
	struct kmem_cache_node *n;
	struct slab *slab;struct slab *t;
	...
	if (flags & SO_CPU) {
		...
		for_each_possible_cpu(cpu) {
			struct kmem_cache_cpu *c = per_cpu_ptr(s->cpu_slab,
							       cpu);
			int node;
			struct slab *slab;slab = READ_ONCE(c->slab);
			if (!slab)
				continue;
